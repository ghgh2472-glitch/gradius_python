...
def tab_assignment(data):
    ...
    if role_status:
        ...
        for i, rs in enumerate(role_status):
            ...
            with cols[i % len(cols)]:
                ...
                # 클릭하면 해당 직군 자동 선택
                if pct < 100:
                    if st.button(f"🎯 {rs['role']} 배정", key=f"quick_role_{i}", use_container_width=True):
                        st.session_state['_quick_role'] = rs['role']
                        st.rerun()
    ...
    with col_left:
        ...
        # 직군 선택 (필요인력 카드 클릭 시 자동 선택)
        role_options = [rs['role'] for rs in role_status] if role_status else []
        role_options.append("기타 (직접입력)")
        _quick = st.session_state.pop('_quick_role', None)
        _default_idx = 0
        if _quick and _quick in role_options:
            _default_idx = role_options.index(_quick)
        col_role, col_custom = st.columns([2, 1])
        with col_role:
            sel_role = st.selectbox("배정 직군", role_options, index=_default_idx, key="assign_role")
        with col_custom:
            if sel_role == "기타 (직접입력)":
        ...
        # ── 신규 인력 직접 입력 (인력풀에 없는 경우) ──
        st.markdown("##### ✍️ 신규 인력 직접 입력")
        st.caption("💡 인력풀에 없는 신규 인력을 등록하고 바로 배정합니다. STAFF 시트에도 자동 저장됩니다.")
        with st.expander("➕ 신규 인력 입력", expanded=False):
            ...
            with nc4:
                new_job = st.text_input("가능직무", value=sel_role if sel_role else "", key="new_staff_job")
            with nc5:
            ...
            with nc7:
                new_pay = st.number_input("지급단가", value=default_rate, step=10000, key="new_staff_pay")
            with nc8:
                new_days = st.number_input("근무일수", value=default_days, min_value=1, key="new_staff_days")

            _save_to_staff = st.checkbox("STAFF 시트에도 등록", value=True, key="save_new_to_staff")

            if st.button("✅ 신규 인력 배정 추가", type="primary", use_container_width=True, key="add_new_staff_btn"):
                if not new_name.strip():
                    ...
                    st.session_state.assign_cart.append({
                        '인력명': new_name.strip(),
                        '구분': '외부',
                        '직무': new_job if new_job else sel_role,
                        '지급단가': int(new_pay),
                        '근무일수': int(new_days),
                        '총지급액': int(new_pay) * int(new_days),
                    })
                    ...
                    if _save_to_staff:
                        staff_data = {
                            '이름': new_name.strip(),
                            '연락처': new_phone.strip() if new_phone else '',
                            '성별': new_gender,
                            '나이': str(new_age),
                            '가능직무': new_job if new_job else sel_role,
                            '이동가능지역': new_region.strip() if new_region else '',
                            '추천도': '일반',
                        }
                        if db.add_new_staff(staff_data):
                            st.success(f"✅ {new_name} — 배정 추가 + STAFF 등록 완료!")
                        else:
                            st.warning(f"⚠️ {new_name} — 배정 추가됨 (STAFF 등록 실패 또는 이미 존재)")
                    else:
                        st.success(f"✅ {new_name} — 배정 추가 완료!")
                    st.rerun()
