...
def apply_styles():
    ...
    <style>
        ...
        .proj-card {
            ...
            border-left: 4px solid #94a3b8; min-height: 120px;
        }
...
def show(data):
    ...
    def _render_cards(df_cards, status_class, status_label_class, tab_prefix="all"):
        """카드 그리드 렌더링 — 한 행에 4개"""
        if df_cards.empty:
        ...
        for row_chunk in rows_data:
            ...
            for col_idx, (_, row) in enumerate(row_chunk.iterrows()):
                with cols[col_idx]:
                    ...
                    _meta_parts = []
                    if _qty: _meta_parts.append(f"👥{_qty}명")
                    ...
                    """, unsafe_allow_html=True)
                    if st.button("선택", key=f"sel_{tab_prefix}_{_label}"):
                        # 중간 키로 카드 선택 저장 → rerun 시 selectbox에 반영
    ...
    with st.expander(f"📋 프로젝트 대기열 ({_cnt_total}건)", expanded=(_cnt_total > 0 and sel_p == "(신규작성)")):
        ...
        with tab_new:
            _render_cards(pending_new, "status-new", "new", "new")
        with tab_edit:
            _render_cards(pending_edit, "status-edit", "edit", "edit")
        with tab_contracted:
            _render_cards(pending_contracted, "status-contracted", "contracted", "cont")
