...
def get_settlement_overview(df_settlement):
    ...
    if df_settlement.empty:
        return {"공급가액": 0, "총청구액": 0, "받은금액": 0, "미수금액": 0, "수금률": 0}
    ...
    try:
        ...
        for _, row in df_settlement.iterrows():
            if col_invoice:
                total_supply += safe_int(row.get(col_invoice, 0))
            if col_tax:
                total_tax += safe_int(row.get(col_tax, 0))
            if col_paid:
                total_paid += safe_int(row.get(col_paid, 0))
...
def get_unpaid_companies(df_settlement, top_n=5):
    """미수금 있는 업체 Top N (금액순) — 공급가액+부가세-받은금액 > 0 기준"""
    ...
    if not col_client or not col_supply: return pd.DataFrame()
    
    try:
        df = df_settlement.copy()
        df['_공급'] = df[col_supply].apply(safe_int) if col_supply else 0
        df['_부가세'] = df[col_tax].apply(safe_int) if col_tax else 0
        df['_받은'] = df[col_paid].apply(safe_int) if col_paid else 0
        df['미수금'] = (df['_공급'] + df['_부가세'] - df['_받은']).clip(lower=0)
        df = df[df['미수금'] > 0]  # 미수금 > 0인 것만
        
        # 업체별 그룹핑 (같은 업체의 여러 건을 합산)
        grouped = df.groupby(df[col_client].astype(str).str.strip()).agg(
            미수금액=('미수금', 'sum'),
            건수=('미수금', 'count'),
        ).reset_index()
        grouped.columns = ['업체', '미수금액', '건수']
        grouped = grouped.sort_values('미수금액', ascending=False).head(top_n).reset_index(drop=True)
        grouped['순위'] = grouped.index + 1
        return grouped[['순위', '업체', '미수금액', '건수']]
    except Exception as e:
